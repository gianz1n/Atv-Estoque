...
#cadastro produtos - CREATE
    def cadastrar_produto(self, cod, nome, fabricante, quantidade=0):
...
#listar produtos
    def listar_produto(self, cod):
        if cod == '':
            ...
            lista = self.meu_cursor.fetchall()
            for i in lista:
                list1 = (f' {i} ').replace(',', ' | ').replace("'", '♦').replace('(', '╠{').replace(')', '}╣')
                ...
                lista = self.meu_cursor.fetchall()
                for i in lista:
                    lista = (f' {i} ').replace(',', ' | ').replace("'", '♦').replace('(', '╠{').replace(')', '}╣')
                    return lista
